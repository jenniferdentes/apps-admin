...
function TaskPanel({ label, task, onChange, onRemove }: TaskPanelProps) {
  ...
  return (
    <Box sx={{ border: '1px solid #E2E8F0', borderRadius: 1, overflow: 'hidden', mb: 3, maxWidth: 900 }}>
      {/* Header */}
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', bgcolor: '#F8FAFC', px: 2, py: 1.5, borderBottom: '1px solid #E2E8F0' }}>
        ...
        <Tooltip title="Remove this task template">
          <IconButton size="small" onClick={onRemove} sx={{ color: '#94A3B8', '&:hover': { color: '#EF4444', bgcolor: '#FEF2F2' } }}>
      ...
      <Box sx={{ p: 2.5 }}>
        ...
        <Box sx={{ mb: 3 }}>
          ...
          <Box onClick={() => videoRef.current?.click()}
            sx={{ border: '2px dashed #CBD5E1', borderRadius: 1, p: 3, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 0.5, cursor: 'pointer', mb: 2, '&:hover': { borderColor: '#94A3B8' } }}>
...
export default function Step5Tasks({ data, update }: Props) {
  ...
  return (
    <Box sx={{ maxWidth: 1200 }}>
      ...
      {addedTypes.length === 0 && (
        <Box sx={{ border: '2px dashed #D0D5DD', borderRadius: 1, py: 4, maxWidth: 900, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1.5, mb: 3, bgcolor: '#F7F8FC' }}>
      ...
      {availableTypes.length > 0 && (
        <>
          <Button
            ...
            sx={{ borderColor: '#D0D5DD', color: '#1B2A3B', fontWeight: 500, fontSize: '0.875rem', textTransform: 'none', borderRadius: 1, boxShadow: 'none' }}
          >
            Add Task
          </Button>
          <Menu
